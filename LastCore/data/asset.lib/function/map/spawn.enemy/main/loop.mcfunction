...
## 乱数
	execute store result score #ListSize Temp if data storage asset:tmp Enemies[]
	execute store result score #Rand Temp run random value 1..
	scoreboard players operation #Rand Temp %= #ListSize Temp
	execute store result storage asset:tmp Num int 1 run scoreboard players get #Rand Temp

## 敵を召喚
	function asset.lib:map/spawn.enemy/main/spawn with storage asset:tmp
